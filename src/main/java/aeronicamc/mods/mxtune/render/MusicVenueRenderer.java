...
public class MusicVenueRenderer
{
    ...
    public static void render(MatrixStack pMatrixStack, IRenderTypeBuffer.Impl pBuffer, ActiveRenderInfo pActiveRenderInfo, ClippingHelper pClippingHelper)
    {
        ...
        MusicVenueProvider.getMusicVenues(level).filter(areas -> isToolInHotBar(player)).ifPresent(
                areas -> areas.getVenueList().stream().filter(Objects::nonNull)
