...
/**
 * 
 * Plays a sound on either the client (only that one player hears the sound) or
 * server side (everyone nearby can hear the sound).
 * 
 * This is useful if, for example, the player levels up on the server - you may
 * not want everyone to hear the level up sound, so you send this packet to the
 * client.
 * 
 * Another example, the player is performing some action on the client side,
 * such as pressing a key or fiddling in a GUI, that makes a sound which
 * everyone nearby should hear - simply send this packet to the server and
 * voilá.
 *
 */
public class PlaySoundPacket extends AbstractMessage<PlaySoundPacket>
{
     ...
     * (yes, that is indeed slightly wasteful when sending this packet to the
     * client...)
     */
    ...
    /**
     * Use only when sending to the SERVER to use the entity's coordinates as
     * the center; if sent to the client, the position coordinates will be
     * ignored.
     */
    ...
    /**
     * Use only when sending to the CLIENT - the sound will play at the player's
     * position
     */
    ...
    public void process(EntityPlayer player, Side side)
    {
        ...
        } else
        {
            ...
            player.worldObj.playSound(x, y, z, soundEvent, SoundCategory.AMBIENT, volume, pitch, false);
        }
    }
}
