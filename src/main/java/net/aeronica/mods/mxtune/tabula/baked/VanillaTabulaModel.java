package net.aeronica.mods.mxtune.tabula.baked;
...
import net.aeronica.mods.mxtune.tabula.math.Matrix;
...
public class VanillaTabulaModel implements IModel {
    ...
    private void build(Matrix mat, Builder<BakedQuad> builder, VertexFormat format, List<TabulaCubeContainer> cubeContainerList, TextureAtlasSprite sprite) {
        for (TabulaCubeContainer cube : cubeContainerList) {
            ...
            float y1 = (y + s + h);
            float z1 = (z + s + d);
            boolean isTxMirror = cube.isTxMirror();
            if (isTxMirror) {
                float x1_ = x1;
                x1 = x0;
                x0 = x1_;
    ...
    private boolean hasTransparency(TabulaCubeContainer cube, TextureAtlasSprite sprite) {
        ...
        if (frameCount > 0) {
            for (int i = 0; i < frameCount; i++) {
                ...
                int[] textureOffset = cube.getTxOffset();
                int[] dimensions = cube.getDimensions();
