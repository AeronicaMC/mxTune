...
public final class ModelMultiTex implements IModel
{
    public static final ModelMultiTex INSTANCE = new ModelMultiTex(of());
    ...
    public ModelMultiTex(ImmutableList<ResourceLocation> textures)
    {
        this(textures, ItemOverrideList.NONE);
    }

    public ModelMultiTex(ImmutableList<ResourceLocation> textures, ItemOverrideList overrides)
    {
        this.textures = textures;
        this.overrides = overrides;
    }

    public ModelMultiTex(ModelBlock model)
    {
    ...
    public ModelMultiTex retexture(ImmutableMap<String, String> textures)
    {
        ImmutableList.Builder<ResourceLocation> builder = ImmutableList.builder();
        ...
        return new ModelMultiTex(builder.build(), overrides);
    }
    ...
    public IBakedModel bake(IModelState state, final VertexFormat format, Function<ResourceLocation, TextureAtlasSprite> bakedTextureGetter)
    {
        ...
        builder.addAll(getQuadsForSprite(0, particle, format, transform));
        BakedItemModelMultiTex baseModel = new BakedItemModelMultiTex(builder.build(), particle, map, overrides, identity);
        baseModel.addMultiTexModel("layer" + 0, baseModel);
        for(int i = 1; i < textures.size(); i++)
        {
            ImmutableList.Builder<BakedQuad> builder2 = ImmutableList.builder();
            TextureAtlasSprite sprite = bakedTextureGetter.apply(textures.get(i));
            builder2.addAll(getQuadsForSprite(i, sprite, format, transform));
            baseModel.addMultiTexModel("layer" + i, new BakedItemModelMultiTex(builder2.build(), particle, map, overrides, identity));
        }
        return baseModel;
    }
    ...
    public static ImmutableList<BakedQuad> getQuadsForSprite(int tint, TextureAtlasSprite sprite, VertexFormat format, Optional<TRSRTransformation> transform)
    {
        ...
        ModelMultiTex.FaceData faceData = new ModelMultiTex.FaceData(uMax, vMax);
